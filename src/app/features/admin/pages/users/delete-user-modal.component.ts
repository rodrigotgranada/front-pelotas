          ...
          <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                <h3 class="text-lg font-semibold leading-6 text-slate-900" id="modal-title">Desligar Usuário</h3>
                <div class="mt-2 text-left">
                  <p class="text-sm text-slate-600 mb-2">
                    O que você deseja fazer com <b>{{ user.firstName }} {{ user.lastName }}</b>?
                  </p>
                </div>
                ...
                <div class="mt-4 space-y-4 text-left">
                  ...
                  <div class="rounded-lg border border-slate-200 bg-slate-50 p-4 relative">
                    <h4 class="text-sm font-semibold text-slate-900 flex items-center gap-2">
                      Inativação Padrão (Recomendado)
                    </h4>
                    <p class="text-xs text-slate-600 mt-1">O usuário perderá o acesso imediatamente. No entanto, seus dados continuarão na base ocultos para auditoria.</p>
                    <button 
                      ...
                      [disabled]="loading()"
                    >
                      <span [class.opacity-0]="loading()">Inativar Acesso (Soft Delete)</span>
                  ...
                  @if (session.me()?.roleCode === 'owner') {
                    <div class="rounded-lg border border-rose-200 bg-rose-50 p-4 relative">
                      ...
                      <p class="text-xs text-rose-700 mt-1 font-medium">Isso excluirá os dados e fotos desse usuário pra sempre. Essa ação é <b>irreversível</b>.</p>
                      ...
                      <div class="mt-3 space-y-2">
                        ...
                        >
                          <span [class.opacity-0]="loading()">Sim, apagar os dados definitivamente</span>
                          @if (loading()) { <app-spinner [inline]="true" size="sm" class="absolute inset-0 flex items-center justify-center" /> }
                        </button>
                      </div>
                    </div>
...
export class DeleteUserModalComponent {
  ...
  async confirmSoftDelete() {
    ...
    try {
      await firstValueFrom(this.usersService.delete(this.user.id));
      this.toastService.showSuccess('Usuário inativado com sucesso.', 'Sucesso');
      this.deleted.emit();
    } catch (err) {
      this.toastService.showApiError(err, 'Falha ao inativar usuário');
    } finally {
  ...
  async confirmHardDelete() {
    ...
    this.loading.set(true);
    try {
      await firstValueFrom(this.usersService.hardDelete(this.user.id));
      this.toastService.showSuccess('Usuário removido definitivamente.', 'Sucesso');
      this.deleted.emit();
    } catch (err) {
